...
if __name__ == "__main__":
    ...
    flask_app.run(host="0.0.0.0", port=PORT)            raise ValueError("Не удалось загрузить страницу.")
        ...
        if not text:
            raise ValueError("Не удалось извлечь читаемый текст. Возможно, это не статья.")

        # Парсинг XML от trafilatura
        wrapper = f"<doc>{text}</doc>"
        tree = etree.fromstring(wrapper.encode("utf-8"))
